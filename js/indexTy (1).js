...
function loadPage() {
  ...
  inputSearch.addEventListener("keyup", () => {
    if (event.key == "Enter") {
      ...
      window.open("https://tu.market/search-offer/query?query=" + stringSearch)
    }
  });
...
function sidebarOther(_mJson, _mTabName, _mOnClick) {
  ...
  jsonRequest.onload = function() {
    ...
    let sidebarTab = document.createElement("div");
    textHTML += `<h2 class="sidebarH2">${_mTabName}</h2>`;
    textHTML += `<a href="#" onclick=clickTab(${_mOnClick})><div class="sidebarOther">`;
    textHTML += `<img src="${catObject[randomNum].linkPhotoMidl}" class="sidebarImg" id="sidebarImg">`
    textHTML += `<span class="sidebarPrice">${catObject[randomNum].price} руб.</span>`;
    textHTML += `<span class="sidebarNote">${catObject[randomNum].nameTU.toUpperCase()}</span>`;
    textHTML += `</div></a>`;
    sidebarTab.innerHTML = textHTML;
    sideBar.appendChild(sidebarTab);
  }
